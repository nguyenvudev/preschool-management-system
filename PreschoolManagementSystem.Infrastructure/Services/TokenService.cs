...
namespace PreschoolManagementSystem.Infrastructure.Services
{
    public class tokenService : ITokenService
    {
        ...
        new Claim("PreschoolId", user.PreschoolId.ToString()),
        new Claim(JwtRegisteredClaimNames.Jti, Guid.NewGuid().ToString())
    };
    
         var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_configuration["Jwt:Key"] ?? throw new InvalidOperationException ("jwt chua duoc cau hinh") ));
        ...
        public ClaimsPrincipal GetPrincipalFromExpiredToken(string token)
        {
          // Cấu hình tham số xác thực token
            var tokenValidationParameters = new TokenValidationParameters
            {
                ...
                ValidateLifetime = false // Không kiểm tra thời hạn token
            };
